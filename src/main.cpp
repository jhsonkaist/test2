...
// class CMesh    : public vcg::tri::TriMesh< std::vector<MyVertex>, std::vector<MyFace> , std::vector<MyEdge>  > {};
...
void Usage()
{
  printf("\nUsage:  "\
                                        ...
                                        " -x @		perform watermark extracting\n"\
                                        " -c		set the watermark capacity\n"\
                                        " -r @		read the watermark bit from file @\n"\
                                        " -a #		perform alternative process with # bit\n"\
                    ...
                    //                     "  -H         Use an Hashed Uniform Grid as Search Structure\n"\
                                        "\n"
                                        "Default options are to embed watermark with random bits \n"
                                        "and save the watermarked mesh and the watermark.\n"
                                        );
  exit(-1);
}
...
std::string SaveFileName(const std::string &filename)
{
 ...
 std::string fileout;
 if (ExtractFlag){
	 //std::string fileout=filename.substr(0,pos)+"_ext.off";
 	fileout=filename.substr(0,pos)+"_ext.off";
 }
 else {
 	 //std::string fileout=filename.substr(0,pos)+"_emb.off";
 	fileout=filename.substr(0,pos)+"_emb.off";
 }
 return fileout;
}

// simple aux function that compute the name for the file containing the stored computations
std::string SaveWMName(const std::string &filename)
{
 int pos=filename.find_last_of('.',filename.length());
 std::string fileout=filename.substr(0,pos)+"_wm.txt";
 return fileout;
}
...
{
  int err = tri::io::ImporterOFF<CMesh>::Open(m,filename);
  if(err) {
...
int main(int argc, char**argv)
{
  ...
  //      flags |= SamplingFlags::USE_STATIC_GRID;
    ...
    string OutMeshName=SaveFileName(argv[1]);
    string OutBitName=SaveWMName(argv[1]);
 ...
 //    bbox.Add(S2.bbox);
    ...
    // save files.
    tri::io::ExporterOFF<CMesh>::Save (S1, OutMeshName.c_str(), true);
    // save wm bits.
    /* here */
